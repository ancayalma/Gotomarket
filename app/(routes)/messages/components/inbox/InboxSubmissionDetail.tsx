        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-zinc-800/60 bg-zinc-900/80 backdrop-blur-xl sticky top-0 z-10">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-[9px] h-4 px-1.5 border-violet-500/30 text-violet-400 bg-violet-500/10">
                            Form
                        </Badge>
                    </div>
                </div>
                ...
                {!submission.converted_lead_id && !submission.lead_id && (
                    submission.form?.project_id ? (
                        <Button
                            ...
                            className="gap-1.5 h-7 text-[11px] bg-indigo-600 hover:bg-indigo-500 text-white"
                        >
                ...
                {submission.is_deleted ? (
                    <div className="flex items-center gap-1.5">
                        <Button variant="outline" size="sm" onClick={() => onRestore(submission.id)} disabled={isDeleting} className="h-7 text-[11px] gap-1 border-zinc-700 text-zinc-300 hover:text-white">
                    ...
                    <div className="flex items-center gap-0.5">
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800" onClick={() => onArchive(submission.id)} disabled={isDeleting}>
                                    <Archive className="h-3.5 w-3.5" />
                                </Button>
                            </TooltipTrigger>
                            <TooltipContent className="bg-zinc-800 border-zinc-700 text-zinc-200 text-xs">Archive</TooltipContent>
                        </Tooltip>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-zinc-400 hover:text-red-400 hover:bg-red-500/10" onClick={() => onDelete(submission.id)} disabled={isDeleting}>
                                    <Trash2 className="h-3.5 w-3.5" />
                                </Button>
                            </TooltipTrigger>
                            <TooltipContent className="bg-zinc-800 border-zinc-700 text-zinc-200 text-xs">Trash</TooltipContent>
                        </Tooltip>
                    </div>
                )}
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        {submission.data?.email && (
                            <div className="text-[11.5px] text-zinc-500">{submission.data.email}</div>
                        )}
            ...
            <ScrollArea className="flex-1 p-4">
                <div className="space-y-4">
                    <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.15em]">
                        Submitted Data
                    </h4>
                    <div className="space-y-2.5">
                        {Object.entries(submission.data || {}).map(([key, value]) => (
                            <div key={key} className="rounded-xl bg-zinc-800/30 border border-zinc-700/30 p-3">
                    ...
                    {(submission.source_url || submission.ip_address) && (
                        <div className="mt-6 pt-4 border-t border-zinc-800/40">
                            <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.15em] mb-2">
                                Metadata
                            </h4>
                            <div className="text-[12px] text-zinc-400 space-y-1">
                                {submission.source_url && <div><span className="text-zinc-500">Source:</span> {submission.source_url}</div>}
                                {submission.ip_address && <div><span className="text-zinc-500">IP:</span> {submission.ip_address}</div>}
                            </div>
                        </div>
                    )}
                </div>
