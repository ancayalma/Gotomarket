...
import { LayoutDashboard, TableProperties, FileText, Settings, Pen, CreditCard, Database } from "lucide-react";
    ...
    return (
        <nav className="flex-1 py-4 px-3 flex flex-col gap-0.5">
            ...
            <Link href="/transform" className={`flex items-center gap-2.5 px-2.5 py-2 rounded-md text-[12px] font-medium transition-colors group cursor-pointer ${
                !activeTool ? "bg-white/[0.06] text-white" : "text-white/70 hover:text-white hover:bg-white/[0.04]"
            }`}>
                <LayoutDashboard className={`w-3.5 h-3.5 transition-colors ${!activeTool ? "text-orange-400" : "text-white/30 group-hover:text-orange-400"}`} />
                Extraction Studio
            </Link>
